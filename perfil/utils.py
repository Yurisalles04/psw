...
def calcula_equilibro_financeiro():
    gastos = Valores.objects.all()
    gastos_essenciais = gastos.filter(data__month = datetime.now().month).filter(tipo='S').filter(categoria__essencial = True)
    gastos_nao_essenciais = gastos.filter(data__month = datetime.now().month).filter(tipo='S').filter(categoria__essencial = False)
